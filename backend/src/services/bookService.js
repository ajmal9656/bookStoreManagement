...
export const createBook = async ({ title, authorId, price, stock }) => {
  const authorExists = await checkFieldValueExist("Author", "id", authorId);
  ...
  do {
    isbn = generateISBN();
  } while (await checkFieldValueExist("Book", "isbn", isbn));
...
export const updateBookStock = async (id, { operation, quantity }) => {
  ...
  if (operation === "decrease" && quantity > book.stock) {
  ...
  return await bookRepository.updateBookStock(id, operation, quantity);
};
